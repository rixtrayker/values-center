						...
						<table id="table_id" class="table table-bordered data_table">
							...
							<tbody>
									...
									<tr>
                                            ...
                                            <td>{{$record->serial}}</td>
                                            <td>{{$record->student->name}} - {{$record->student->eduCenter->name}}</td>
                                            <td>{{$record->test_center}}</td>
                                            <td>{{$record->eduCenter->name}}</td>
                                            <td>{{$record->user->name}}</td>
                                            <td> @if($record->status === 0 ) Booked @elseif ($record->status === 1) Paid @elseif ($record->status === 2) Booked & Paid @endif </td>
                                            <td>{{count($record->payments)}}</td>
                                            <td>
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    ...
                                                    <form role="form" action="{{ route('registrations.destroy',$record->id) }}" class="" method="POST">
