...
<div class="card card-custom">
    <div class="card-header">
        <div class="card-title">
            ...
            <h3 class="card-label">Lectures</h3>
        </div>
        <div class="card-toolbar">
            <!--begin::Button-->
            <a href="{{ route('lectures.create') }}" class="btn btn-primary font-weight-bolder">
						...
						<table id="table_id" class="table table-bordered data_table">
							...
							<tbody>
									...
									<tr>
                                            ...
                                            <td>{{$record->month}}</td>
                                            <td>{{$record->cost}}</td>
                                            <td>{{$record->day_one}}</td>
                                            <td>{{$record->day_two}}</td>
                                            <td>{{$record->number_of_sessions}}</td>
                                            <td>{{$record->number_of_sessions - $record->left_sessions}}</td>

                                            <td>
                                                <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown"
                                                aria-haspopup="true" aria-expanded="true">Action</button>
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
												    <a class="dropdown-item" href="{{route('lectures.edit', $record->id)}}"><i class="fa fa-edit"></i> &nbsp; Edit</a></a>
                                                    ...
                                                    <form role="form" action="{{ route('lectures.destroy',$record->id) }}" class="" method="POST">
